...
class StationCongestionModel:
    def __init__(self, models_dir="models/congestion_predictor"):
        self.models_dir = models_dir
        self.preprocessor = DataPreprocessor(level_name="station")
    ...
    def load_model(self):
        """
        Loads preprocessor and K-Fold ensembles.
        """
        try:
            self.preprocessor.load_preprocessor(self.models_dir)
            self.ensemble_15 = joblib.load(os.path.join(self.models_dir, "station_model_15.pkl"))
            self.ensemble_30 = joblib.load(os.path.join(self.models_dir, "station_model_30.pkl"))
            self.ensemble_60 = joblib.load(os.path.join(self.models_dir, "station_model_60.pkl"))
            self.initialized = True
        except Exception as e:
            pred_logger.error(f"Failed to load Station Congestion Model: {e}")
            self.initialized = False

    def predict_congestion(self, df_tick: pd.DataFrame) -> list:
        """
        Predicts future station congestion and calculates ensemble standard deviation confidence limits.
        """
        ...
        for idx in range(len(df_tick)):
            station_id = int(df_tick.iloc[idx]["station_id"])
            c_15 = max(0.0, min(100.0, float(p_15_mean[idx])))
            c_30 = max(0.0, min(100.0, float(p_30_mean[idx])))
            c_60 = max(0.0, min(100.0, float(p_60_mean[idx])))
            ...
            lower_bound = max(0.0, c_30 - 1.96 * std_val)
            upper_bound = min(100.0, c_30 + 1.96 * std_val)

            results.append({
                "station_id": station_id,
                "congestion_15": round(c_15, 2),
                "congestion_30": round(c_30, 2),
                "congestion_60": round(c_60, 2),
